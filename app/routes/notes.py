...
def create_note(
    ...
    return repository.create_note(db, note_data, owner_id=current_user.id)
...
def list_notes(
    ...
    return repository.get_notes(db, owner_id=current_user.id, skip=skip, limit=limit)
...
def get_note(
    ...
    note = repository.get_note_by_id(db, note_id, owner_id=current_user.id)
    if not note:
...
def update_note(
    ...
    note = repository.update_note(db, note_id, note_data, owner_id=current_user.id)
    if not note:
        raise HTTPException(status_code=404, detail="Note not found")
    return note
...
def delete_note(
    ...
    if not repository.delete_note(db, note_id, owner_id=current_user.id):
        raise HTTPException(status_code=404, detail="Note not found")
